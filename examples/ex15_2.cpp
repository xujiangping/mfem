...
int main(int argc, char *argv[])
{
...

   {
      ...
      ofstream omesh("Test15_2_read.vtk");  
      omesh.precision(8);  
      mesh.PrintVTK(omesh);  
   }    
...

   {
      ...
      ofstream omeshu("Test15_2_UniformRefinement.vtk");  
      omeshu.precision(8);  
      mesh.PrintVTK(omeshu);  
   }

    Array<Refinement> elementsToRefine1(2); //指定加密单元
    elementsToRefine1[0]=(2,0);
    elementsToRefine1[1]=(2,1);
...

   {
      ...
      ofstream omesh1("Test15_2_FirstRefinement.vtk");  
      omesh1.precision(8);  
      mesh.PrintVTK(omesh1);  
   }

    Array<Refinement> elementsToRefine(2); //指定加密单元
    elementsToRefine[0]=(2,0);
    elementsToRefine[1]=(2,1);

    //Refine the specified elements
    mesh.GeneralRefinement(elementsToRefine,1,1);
 
   {
      cout << "Number of elements after the second refinement: " << mesh.GetNE() << endl;  
      ofstream omesh1("Test15_2_SecondRefinement.vtk");  
      omesh1.precision(8);  
      mesh.PrintVTK(omesh1);  
      // const mfem::Array<int>& root_elements = mesh.GetRoot();
      // cout << "Root element of a specific element: " << root_elements << endl; 
      return 0;
   }   
